...
class ResidualAttentionBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor, key_padding_mask: torch.Tensor = None):
        device = x.device
        # Move key_padding_mask to the correct device
...
class Transformer(nn.Module):
    ...
    def forward(self, input_ids, attention_mask=None):
        # Ensure input tensors are on the same device as the model's parameters
...
def get_transformer_encoder(config_encoder, verbose, **kwargs):
