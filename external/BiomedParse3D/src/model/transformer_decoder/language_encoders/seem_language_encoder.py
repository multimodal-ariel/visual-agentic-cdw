...
class SEEMLanguageEncoder(nn.Module):
    def __init__(
        self,
        tokenizer,
        tokenizer_type,
        ...
        if self.tokenizer_type == "clip":
            self.tokenizer.add_special_tokens({"cls_token": tokenizer.eos_token})
    ...
    def get_text_embeddings(
        ...
        if not is_eval:
            ...
            text_emb = self.forward_language(
                (arbitary_tokens, arbitary_attention_masks), norm=norm
            )
    ...
    def forward_language(self, texts, norm=True):
        x = self.encoder_transformer(*texts)
