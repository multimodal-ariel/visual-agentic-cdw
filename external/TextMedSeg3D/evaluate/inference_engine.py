...
def inference(model, text_encoder, device, testset, testloader, nib_dir):
    ...
    with torch.no_grad():
        ...
        for batch in testloader:
            ...
            # Get affine info for saving in correct space
            transformed_affine = batch.get('transformed_affine', np.eye(4))
            if isinstance(transformed_affine, torch.Tensor):
                transformed_affine = transformed_affine.numpy()
            original_affine = batch.get('original_affine', None)
            if isinstance(original_affine, torch.Tensor):
                original_affine = original_affine.numpy()
            original_shape = batch.get('original_shape', None)

            # Build target (original space) NIfTI for resampling
            resample_to_original = (original_affine is not None and original_shape is not None)
            if resample_to_original:
                if isinstance(original_shape, torch.Tensor):
                    original_shape = tuple(original_shape.tolist())
                elif isinstance(original_shape, (list, np.ndarray)):
                    original_shape = tuple(int(s) for s in original_shape)
                target_nii = nib.Nifti1Image(np.zeros(original_shape, dtype=np.uint8), original_affine)
            ...
            for j, label in enumerate(labels):
                ...
                # Save per-label mask in original image space
                seg_data = prediction[j, :, :, :]
                if resample_to_original:
                    # Create NIfTI in SAT's transformed space, then resample to original
                    seg_in_sat_space = nib.Nifti1Image(seg_data.astype(np.float32), transformed_affine)
                    seg_in_orig = resample_from_to(seg_in_sat_space, target_nii, order=0)  # nearest-neighbor
                    seg_data_orig = (seg_in_orig.get_fdata() > 0.5).astype(np.uint8)
                    segobj = nib.Nifti1Image(seg_data_orig, original_affine)
                else:
                    segobj = nib.nifti2.Nifti1Image(seg_data, transformed_affine)
                nib.save(segobj, f'{nib_dir}/{dataset_name}/seg_{sample_id}/{label}.nii.gz')

            segobj = nib.nifti2.Nifti1Image(results, transformed_affine)
            nib.save(segobj, f'{nib_dir}/{dataset_name}/seg_{sample_id}.nii.gz')

            image = batch['image'].numpy()
            if image.ndim == 4:
                image = image[0, :, :, :]   # h w d
            imgobj = nib.nifti2.Nifti1Image(image, transformed_affine)
